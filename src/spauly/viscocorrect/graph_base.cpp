...
namespace viscocorrect
{
    ...
    GraphCoordsStorage &GraphInternal::InitData()
    {
        ...
        CreateLineCoords(&data_.viscosity_coords, &internal::kProperties.kViscoScale, internal::kProperties.kPitchVisco, internal::kProperties.kStartVisco, true, false);
    ...
    void GraphInternal::CreateLineCoords(GraphLineStorage *coords, const std::map<int, int> *raw_points, const double pitch, const int *startpos, bool scale_on_x, bool use_same_x)
    {
        ...
        GraphCoordType it_total_dist = static_cast<GraphCoordType>((scale_on_x) ? startpos[0] : startpos[1]);

        if (!use_same_x)
        {
            for (const auto &pair : *raw_points)
            {
                it_total_dist += static_cast<GraphCoordType>(pair.second) * scaling_factor_;
    ...
    void GraphImplBase::HideProject()
    {
        set_show_project(false);
    }

    void GraphImplBase::AddCallbackToPlot(std::shared_ptr<std::function<void()>> func, int plot_num)
    {
        switch (plot_num)
        {
        case 0:
    ...
    void GraphImplBase::RemoveCallbackFromPlot(std::shared_ptr<std::function<void()>> func, int plot_num)
    {
        switch (plot_num)
        {
        case 0:
