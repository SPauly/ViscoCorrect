...
namespace ViscoCorrect
{
    namespace Debug
    {
        ...
        void CurveFitting::RenderInPlot()
        {
            for (int i = 0; i < ncurves; i++)
            {
                ImPlot::PlotScatter("raw_points", curves.at(i).xData.data(), curves.at(i).yData.data(), curves.at(i).xData.size());
                ImPlot::PlotLine("fitted", curves.at(i).fittedX.data(), curves.at(i).fittedY.data(), curves.at(i).fittedX.size());
            }
        }
        ...
        void CurveFitting::FitCurve(FullDataCurve &_data)
        {
            ...
            gsl_multifit_nlinear_workspace *workspace = gsl_multifit_nlinear_alloc(gsl_multifit_nlinear_trust, &fittingParams, _datasize, _initial_params->size);
            ...
            for (int i = _data.xData.at(0); i < _data.xData.back(); i++)
            {
                ...
                _data.fittedY.push_back(_data.compressed_data.ModelFunction((double)i, _data.parameters));
            }
